...
public final class IteratorAggregation
{
	 ...
	 * @param bufSize
	 *            size of the internal buffer used by the iterator in 64-bit
	 *            words (per input iterator)
	...
	public static IteratingRLW not(final IteratingRLW x)
	{
		return new IteratingRLW() {
			...
			@Override
			public void discardFirstWords(final long y)
			{
				x.discardFirstWords(y);
			}

			@Override
			public void discardLiteralWords(final long y)
			{
	...
	public static void xorToContainer(final BitmapStorage container, int desiredrlwcount, final IteratingRLW rlwi, final IteratingRLW rlwj)
	{
		while ((rlwi.size() > 0) && (rlwj.size() > 0) && (desiredrlwcount-- > 0))
		{
			while ((rlwi.getRunningLength() > 0) || (rlwj.getRunningLength() > 0))
			{
				...
				{
					final long index = dischargeNegated(container, prey, predator.getRunningLength());
	...
	protected static long discharge(final BitmapStorage container, final IteratingRLW i, final long max)
	{
		long counter = 0;
	 ...
	 * @return how many written
	 */
	protected static long dischargeNegated(final BitmapStorage container, final IteratingRLW i, final long max)
	{
		long counter = 0;
		while (i.size() > 0 && counter < max)
		{
	...
	protected static int inplaceand(final long[] bitmap, final IteratingRLW i)
	{
		int pos = 0;
		long s;
		while ((s = i.size()) > 0)
	...
	protected static int inplaceor(final long[] bitmap, final IteratingRLW i)
	{
	...
	protected static int inplacexor(final long[] bitmap, final IteratingRLW i)
	{
		int pos = 0;
		long s;
		while ((s = i.size()) > 0)
	...
	static void andToContainer(final BitmapStorage container, int desiredrlwcount, final IteratingRLW rlwi, final IteratingRLW rlwj)
	{
		while ((rlwi.size() > 0) && (rlwj.size() > 0) && (desiredrlwcount-- > 0))
		{
			while ((rlwi.getRunningLength() > 0) || (rlwj.getRunningLength() > 0))
	...
	static void andToContainer(final BitmapStorage container, final IteratingRLW rlwi, final IteratingRLW rlwj)
	{
...
{
	CompressedBitSet buffer = new CompressedBitSet();
	LinkedList<IteratingRLW> ll;
	final int bufferSize;

	public BufferedAndIterator(final LinkedList<IteratingRLW> basell, final int bufSize)
	{
		this.ll = basell;
	...
	{
		final BufferedAndIterator answer = (BufferedAndIterator)super.clone();
	...
	public EWAHIterator next()
	{
		...
		if (this.ll.size() > 2)
		{
			final Iterator<IteratingRLW> i = this.ll.iterator();
			i.next();
			i.next();
			final CompressedBitSet tmpbuffer = new CompressedBitSet();
		...
		for (final IteratingRLW aLl : this.ll)
		{
			if (aLl.size() == 0)
			{
				this.ll.clear();
...
{
	final CompressedBitSet buffer = new CompressedBitSet();
	final long[] hardBitmap;
	final LinkedList<IteratingRLW> ll;

	BufferedORIterator(final LinkedList<IteratingRLW> basell, final int bufSize)
	{
		this.ll = basell;
	...
	{
		final BufferedXORIterator answer = (BufferedXORIterator)super.clone();
		answer.buffer = this.buffer.clone();
	...
	public EWAHIterator next()
	{
		...
		while (i.hasNext())
		{
			final IteratingRLW rlw = i.next();
			if (rlw.size() > 0)
			{
				final int eff = IteratorAggregation.inplaceor(this.hardBitmap, rlw);
				if (eff > effective)
				{
					effective = eff;
				}
...
class BufferedXORIterator implements CloneableIterator<EWAHIterator>
{
	...
	BufferedXORIterator(final LinkedList<IteratingRLW> basell, final int bufSize)
	{
		this.ll = basell;
		this.hardbitmap = new long[bufSize];
	}
	...
	public EWAHIterator next()
	{
		...
		while (i.hasNext())
		{
			...
			if (rlw.size() > 0)
			{
				final int eff = IteratorAggregation.inplacexor(this.hardbitmap, rlw);
				if (eff > effective)
				{
					effective = eff;
				}
